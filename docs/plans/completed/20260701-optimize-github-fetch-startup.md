...
- [x] версии/статусы/карточки по-прежнему корректно отображаются (юнит-тесты рендера зелёные).
- [x] `cache.json` после параллельного старта содержит записи по всем repo (нет потерь) — покрыто тестами Task 1–2.
- [x] на каждый тул нет повторного `fetchRepoInfo` (проверка по коду/тесту Task 3).
- [x] установленная версия рендерится независимо от сети (Task 4).
