...
- [ ] Удалить строку `"Press [c] to load changelog"` из `renderCard()`
- [ ] В `fetchRepoCardCmd()` — включить загрузку changelog body сразу (убрать lazy-load через `[c]`)
- [ ] Если загрузка changelog занимает время — показывать `"Loading changelog…"` вместо пустого блока
- [ ] Убедиться, что при смене инструмента в левой панели changelog сбрасывается и загружается заново
- [ ] Проверить, что пустой блок changelog не занимает лишнего вертикального пространства когда данных нет
