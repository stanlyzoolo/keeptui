...
**Tool-list rows** (`renderLeftContent`) follow "one glyph, one meaning" in the width-1 marker column: the selected row always carries `▸` — peach (`SelectionBarStyle`) with the name in bold peach (`SelectedNameStyle`) while the list is focused, dim (`SelectionBarDimStyle`) otherwise — so the selection never disappears when focus moves to brief/help. Non-selected rows carry a status edge `▎` when the tool deviates from active (`statusEdge`: orange for trying, muted for inactive; active is the norm and unmarked); `▸` takes priority over the edge on the selected row. A tool whose installed version is older than the latest release gets a ` ↑` (`UpdateAvailableStyle`) after the name. All three glyphs are single-cell and non-East-Asian-Ambiguous, keeping row-width math stable.
...
- **Card versions**: `renderCard`'s `[info]` section shows `installed:` from `m.versions[name].Installed` (`installed: not found` in dim `DescStyle` when local detection came up empty); the section's gate includes `installed != ""`, so a locally installed tool with no `GitHub` ref still renders it. When `hasUpdate(name)` (installed older than latest per `version.IsNewer`) the `latest:` value renders in `UpdateAvailableStyle` with a ` ↑` suffix — the same glyph the tools list appends to the row. Versions live in the card only; the 30-column list never shows version strings.
- **Help panel title**: `renderHelp` insets the current mode's title (`--help` or `man`, per `m.helpMode`) into the panel's top border via `insetPanelTitle` (render.go) — an ANSI-safe splice over the already-rendered frame (`ui`'s `truncateVisible` is unexported; the helper remeasures with `stripANSI` and rebuilds the top line), colored like the border (focus-aware) and dropped when the panel is too narrow. Brief and tools keep untitled borders.
...
- **Overlay compositing**: the API-status overlay is the single `ui.PlaceOverlay` caller (a centered fg-over-bg compositor). `PlaceOverlay` dims the whole visible background — original styling is stripped and repainted with `OverlayDimStyle` (`ColorDim`) — so the modal is the only full-color element. Covered rows get the dim inside `overlayLine` *after* `truncateVisible`/`dropVisible`, because those helpers `StripANSI` the bg and would erase a pre-applied dim from the modal's side margins.
- **Mouse policy** (`handleMouse` in `render.go`, dispatched from `Update()` before the mode switch, gated inside the function): wheel scrolling works in every mode; while the `[L]` overlay is visible (`apiOverlayVisible()`) all mouse input is a no-op, and before the first `WindowSizeMsg` (`!m.ready`) too. Clicks that change selection or focus fire only in `modeNormal` — otherwise a click would move `selectedMeta()` under an open note/tags/rename editor and retarget the commit. A click that changes the selected tool goes through the same `selectMeta` helper as the keyboard `j`/`k` path, including the auto-fetch; a click anywhere in the tools panel (row or empty area) focuses it, matching brief/help.
