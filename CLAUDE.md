...
- **Tool-list search (`/` in `focusTools`)** is a commit/rollback transaction over `modeSearch`. `case "/"` captures `m.searchPrevName` (the selected tool's name; empty when the list is empty) before entering the mode, and `filteredMeta()` narrows the list live as the query changes (typing resets `metaSelected` to 0 — first match highlighted, marker visible during search). Inside the mode: `↑`/`↓` move the highlight through the filtered list (modular wrap, full `j`/`k` parity incl. `calcVpHeight()` + `autoFetchCmdsForSelected()`; **never** forwarded to the textinput, so the query text is untouched — with zero matches they are consumed as no-ops); `enter` commits — exits to `modeNormal`, clears the query, remaps the cursor onto the **full** list by name via `indexOfMeta(mt.Name)` (the filter is gone once the mode is normal), moves focus to `focusBrief`, and fires `autoFetchCmdsForSelected()` (no matches → no-op, search stays open); `esc` rolls back — full list with the cursor restored via `indexOfMeta(m.searchPrevName)` (fallback 0 when that tool was untracked mid-search). Both exits clear `searchPrevName`. `indexOfMeta(name)` lives next to `filteredMeta()` in `model.go`; the status bar echoes the live query plus `[enter] open  [↑/↓] move  [esc] cancel` hints.
