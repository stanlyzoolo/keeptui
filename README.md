...
keys track https://github.com/owner/repo       из GitHub-ссылки (имя и github выводятся автоматически)
...
Если первым аргументом передать GitHub-ссылку (`https://github.com/owner/repo`, с `.git`, без схемы или в SSH-форме `git@github.com:owner/repo.git`), `keys` подставит короткое имя инструмента (`repo`) в `name` и нормализованный `github.com/owner/repo` в поле `github`. Флаг `--github` имеет приоритет над ссылкой.
