...
- Remove hardcoded `leftWidth = 22` constant
- Implement `calcPanelWidths()` to return three widths: `(toolsW, briefW, helpW)` based on 20%-40%-40% ratio
  - Account for 2 border chars per panel = 6 chars overhead
  - Formula: `available = width - 6`; `toolsW = available * 0.2`; `briefW = available * 0.4`; `helpW = available * 0.4`
  - Ensure minimum widths (toolsW >= 15, briefW >= 30, helpW >= 30)
- Update `calcVpHeight()` to account for new layout (currently uses `height - 10` for content)
