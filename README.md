...
- **Поиск** — `/` в панели tools фильтрует список (`↑` / `↓` — по совпадениям, `enter` — открыть карточку, `esc` — отмена); в панели help ищет по тексту `--help` / `man` с подсветкой совпадений
...
| `/` | поиск по списку: ввод фильтрует, `↑` / `↓` — по совпадениям, `enter` — открыть карточку, `esc` — отмена с возвратом к прежнему выбору (выбор во время поиска — только с клавиатуры, клики не работают) |
