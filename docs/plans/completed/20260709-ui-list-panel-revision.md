...
- [x] обновить CLAUDE.md: разделы про статусы (цикл из трёх), маркер выделения, титул help-панели, охват поиска (+ карточка installed/latest и рендер строк списка)
- [x] README.md — если там упоминаются статусы/горячие клавиши, синхронизировать (цикл статусов, легенда статусов + миграция, поиск по тегам со счётчиком, ASCII-схема с ▸/▎/↑/титулом --help, заметка о титуле help-панели)
