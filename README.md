...
│   helix   │ stars: 82.1k   latest: v0.10.1          │   -es       script  │
│   fzf     │ ── notes ────────────────────────────  │   ...               │
...
- **Версии и changelog** — установленная и актуальная версия с GitHub подгружаются автоматически
- **Поиск по help** — `/` ищет по тексту `--help` / `man` с подсветкой совпадений
...
| `s` | сменить статус (`active → trying → forgotten → archived → active`) |
