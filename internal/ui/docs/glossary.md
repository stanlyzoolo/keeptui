...
- Клавиша `o` открывает страницу релиза в браузере.
...
### Filter Bar — Строка фильтра
- Код: `metaFilter`, `filterLabel`
- Строка `Filter: <статус>` над списком в My Tools.
- Переключается клавишей `f` (цикл) или цифрами `1–4`.
